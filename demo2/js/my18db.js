$(document).ready(function () {
	...
	// Lottery
	if ( $('.lotteryTimer').length ) {
		setInterval(function() {
			$('.lotteryTimer').each(function() {
				var expired = $(this).data('expired');

				var left = Math.floor(expired - new Date() / 1000);
				if ( left < 0 ) {
					var left = 0;
				}
				...
				// update
				if ( $(this).find('.days').length ) {
					$(this).find('.days').html(addZero(2, days));
				}
				if ( $(this).find('.hours').length ) {
					$(this).find('.hours').html(addZero(2, hours));
				}
				if ( $(this).find('.minutes').length ) {
					$(this).find('.minutes').html(addZero(2, minutes));
				}
				if ( $(this).find('.seconds').length ) {
					$(this).find('.seconds').html(addZero(2, seconds));
				}
			});
		}, 1000);
	}
